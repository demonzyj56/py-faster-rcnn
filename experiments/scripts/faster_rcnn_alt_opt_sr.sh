#!/bin/bash
# Usage:
# ./experiments/scripts/faster_rcnn_alt_opt_sr.sh GPU NET DATASET [options args to {train,test}_net.py]
# DATASET is only pascal_voc for now
#
# Example:
# ./experiments/scripts/faster_rcnn_alt_opt_sr.sh 0 VGG_CNN_M_1024 pascal_voc \
...
LOG_PATH="experiments/logs/faster_rcnn_alt_opt_sr"
mkdir -p ${LOG_PATH}
...
time ./tools/train_faster_rcnn_alt_opt.py --gpu ${GPU_ID} \
  ...
  --imdb ${TRAIN_IMDB} \
  --cfg experiments/cfgs/faster_rcnn_alt_opt_sr.yml \
  ${EXTRA_ARGS}
...
time ./tools/rpn_generate.py --gpu ${GPU_ID} \
    --def models/${PT_DIR}/${NET}/faster_rcnn_alt_opt/rpn_test.pt \
    --net ${NET_FINAL} \
    --imdb ${TEST_IMDB} \
    --cfg experiments/cfgs/faster_rcnn_alt_opt_sr.yml \
    ${EXTRA_ARGS}
